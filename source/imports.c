...
static int android_printf_log(const char *fmt, ...) {
  if (!fmt) return 0;
  va_list ap;
  va_start(ap, fmt);
...
static int android_puts_log(const char *text) {
  (void)text;
  return 0;
}
...
int __android_log_vprint(int prio, const char *tag, const char *fmt, va_list va) {
  ...
  va_list copy;
  va_copy(copy, va);
  const int result = vsnprintf(NULL, 0, fmt, copy);
  va_end(copy);
  return result;
}
...
int pthread_attr_getschedparam_fake(const void *a, void *p) {
  (void)a;
  if (p) memset(p, 0, sizeof(int));
  return 0;
}
static int pthread_rwlock_destroy_fake(void **rw) {
  if (rw && *rw) {
    free(*rw);
    *rw = NULL;
  }
...
extern int64_t AAsset_getLength64(void *);
extern int64_t AAsset_seek64(void *, int64_t, int);
...
/* GLES2 exposes program binaries through OES_get_program_binary.  The Switch
 * headers intentionally do not declare the extension entry points, so resolve
 * them from EGL at first use and degrade to an empty cache when unavailable. */
typedef void (*nx_gl_program_binary_oes_fn)(GLuint, GLenum, const void *, GLint);
typedef void (*nx_gl_get_program_binary_oes_fn)(GLuint, GLsizei, GLsizei *,
                                                GLenum *, void *);
static void gl_ProgramBinaryOES_host(GLuint program, GLenum format,
                                     const void *binary, GLint length) {
  static nx_gl_program_binary_oes_fn function;
  static int looked_up;
  if (!looked_up) {
    function = (nx_gl_program_binary_oes_fn)
        eglGetProcAddress("glProgramBinaryOES");
    looked_up = 1;
  }
  if (function) function(program, format, binary, length);
}
static void gl_GetProgramBinaryOES_host(GLuint program, GLsizei capacity,
                                        GLsizei *length, GLenum *format,
                                        void *binary) {
  static nx_gl_get_program_binary_oes_fn function;
  static int looked_up;
  if (!looked_up) {
    function = (nx_gl_get_program_binary_oes_fn)
        eglGetProcAddress("glGetProgramBinaryOES");
    looked_up = 1;
  }
  if (function) {
    function(program, capacity, length, format, binary);
  } else {
    if (length) *length = 0;
    if (format) *format = 0;
  }
...
/* Small Bionic helpers used by Mortar's bundled utility libraries. */
static size_t fwrite_chk_fake(const void *ptr, size_t size, size_t count,
                              FILE *stream, size_t buffer_size) {
  (void)buffer_size;
  return fwrite_fake(ptr, size, count, stream);
}
static size_t malloc_usable_size_safe(void *ptr) {
  return ptr ? malloc_usable_size(ptr) : 0;
}
static void *mremap_fake(void *old_address, size_t old_size, size_t new_size,
                         int flags, ...) {
  (void)flags;
  if (new_size <= old_size) return old_address;
  void *replacement = mmap_fake(NULL, new_size, 3, 0x22, -1, 0);
  if (replacement == (void *)-1 || !replacement) return (void *)-1;
  memcpy(replacement, old_address, old_size);
  munmap_fake(old_address, old_size);
  return replacement;
}
static int pipe2_fake(int fds[2], int flags) {
  (void)flags;
  return pipe_fake(fds);
}
static FILE *popen_fake(const char *command, const char *type) {
  (void)command;
  (void)type;
  errno = ENOSYS;
  return NULL;
}
static int pclose_fake(FILE *stream) {
  (void)stream;
  errno = ECHILD;
  return -1;
}
static int system_fake(const char *command) {
  (void)command;
  errno = ENOSYS;
  return -1;
}
static int setitimer_fake(int which, const void *value, void *old_value) {
  (void)which;
  (void)value;
  if (old_value) memset(old_value, 0, 32);
  return 0;
}
static int getrlimit_fake(int resource, void *limit) {
  (void)resource;
  if (limit) {
...
static int initgroups_fake(const char *user, unsigned group) {
  (void)user; (void)group; return 0;
}
static int setid_fake(unsigned id) { (void)id; return 0; }
static void *getgroup_fake(unsigned id) { (void)id; return NULL; }
static void *getgroupname_fake(const char *name) { (void)name; return NULL; }
static void *getpwnam_compat(const char *name) {
  (void)name;
  return getpwuid_fake(0);
}
static void rewind_fake(FILE *stream) { (void)fseek_fake(stream, 0, SEEK_SET); }
...
DynLibFunction dynlib_functions[] = {
  ...
  { "__fwrite_chk", (uintptr_t)&fwrite_chk_fake },
  ...
  { "malloc_usable_size", (uintptr_t)&malloc_usable_size_safe },
  ...
  // --- printf family ---
  { "printf", (uintptr_t)&android_printf_log }, { "puts", (uintptr_t)&android_puts_log },
  ...
  { "setitimer", (uintptr_t)&setitimer_fake },
  ...
  { "rewind", (uintptr_t)&rewind_fake }, { "tmpfile", (uintptr_t)&tmpfile },
  { "popen", (uintptr_t)&popen_fake }, { "pclose", (uintptr_t)&pclose_fake },
  ...
  { "getpwuid", (uintptr_t)&getpwuid_fake }, { "getpwnam", (uintptr_t)&getpwnam_compat },
  { "getgrgid", (uintptr_t)&getgroup_fake }, { "getgrnam", (uintptr_t)&getgroupname_fake },
  { "getrlimit", (uintptr_t)&getrlimit_fake }, { "getrusage", (uintptr_t)&getrusage_fake },
  { "initgroups", (uintptr_t)&initgroups_fake },
  { "setgid", (uintptr_t)&setid_fake }, { "setuid", (uintptr_t)&setid_fake },
  ...
  // Mortar's HTTP/TLS stack uses numeric-address parsing before DNS fallback.
  // Translate Android AF_INET/AF_INET6 values through the BSD ABI bridge.
  { "inet_aton", (uintptr_t)&ret0_i }, { "inet_pton", (uintptr_t)&nx_inet_pton },
  ...
  { "glBufferData", (uintptr_t)&glBufferData }, { "glBufferSubData", (uintptr_t)&glBufferSubData },
  ...
  { "glGenTextures", (uintptr_t)&glGenTextures }, { "glGenerateMipmap", (uintptr_t)&glGenerateMipmap },
  { "glGetAttribLocation", (uintptr_t)&glGetAttribLocation },
  { "glGetError", (uintptr_t)&glGetError }, { "glGetIntegerv", (uintptr_t)&glGetIntegerv },
  { "glGetProgramiv", (uintptr_t)&glGetProgramiv }, { "glGetProgramInfoLog", (uintptr_t)&glGetProgramInfoLog },
  { "glGetShaderiv", (uintptr_t)&glGetShaderiv }, { "glGetShaderInfoLog", (uintptr_t)&glGetShaderInfoLog },
  { "glGetShaderPrecisionFormat", (uintptr_t)&glGetShaderPrecisionFormat },
  { "glGetString", (uintptr_t)&glGetString }, { "glGetUniformLocation", (uintptr_t)&glGetUniformLocation },
  ...
  { "glUniform2fv", (uintptr_t)&glUniform2fv }, { "glUniform3f", (uintptr_t)&glUniform3f },
  { "glUniform3fv", (uintptr_t)&glUniform3fv }, { "glUniform4f", (uintptr_t)&glUniform4f },
  { "glUniform4fv", (uintptr_t)&glUniform4fv }, { "glUniformMatrix3fv", (uintptr_t)&glUniformMatrix3fv },
  { "glUniformMatrix4fv", (uintptr_t)&glUniformMatrix4fv }, { "glUseProgram", (uintptr_t)&glUseProgram },
  { "glProgramBinaryOES", (uintptr_t)&gl_ProgramBinaryOES_host },
  { "glGetProgramBinaryOES", (uintptr_t)&gl_GetProgramBinaryOES_host },
...
static void   vln_perror(const char*s){  }
...
int resolve_imports(so_module *mod) {
  if (so_relocate(mod) != 0) return -1;
  build_combined();
  return so_resolve(mod, g_combined, g_combined_n, 1);
}
