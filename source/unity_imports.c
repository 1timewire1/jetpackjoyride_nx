/* Supplementary import table for the Unity/IL2CPP engine.
 * It maps newlib/libm/EGL/zlib functions, the port's
 ...
 * Resolved as a second table by resolve_imports().
 */
#include <stdint.h>
#include <stddef.h>
#include <math.h>
...
/* Unity-specific networking helpers. */
extern long        nx_recvmsg(int fd, void *msg, int flags);
extern long        nx_sendmsg(int fd, const void *msg, int flags);
extern unsigned    nx_inet_addr(const char *cp);
extern const char *nx_inet_ntop(int af, const void *src, char *dst, unsigned size);
extern void       *nx_gethostbyname(const char *name);
#define UNET_recvmsg       (uintptr_t)&nx_recvmsg
#define UNET_sendmsg       (uintptr_t)&nx_sendmsg
#define UNET_inet_addr     (uintptr_t)&nx_inet_addr
#define UNET_inet_ntop     (uintptr_t)&nx_inet_ntop
#define UNET_gethostbyname (uintptr_t)&nx_gethostbyname

/* Android thread-naming calls are optional on Switch. */
...
DynLibFunction unity_dynlib_functions[] = {
  ...
  { "gethostbyname", UNET_gethostbyname },
  ...
  { "inet_addr", UNET_inet_addr },
  { "inet_ntop", UNET_inet_ntop },
