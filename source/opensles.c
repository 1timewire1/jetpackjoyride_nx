...
typedef struct Player {
  ...
  int muted;
  float gain; // linear, derived from volume_mb unless muted
...
static SLresult vol_GetMute(void *self, SLboolean *muted) {
  Player *p = CONTAINER(self, Player, vol_vt);
  if (muted) *muted = p->muted ? SL_BOOLEAN_TRUE : SL_BOOLEAN_FALSE;
  return SL_RESULT_SUCCESS;
}
