...
_Static_assert(sizeof(ZipLocalHeader) == 30, "ZIP local header size");
_Static_assert(sizeof(ZipCentralHeader) == 46, "ZIP central header size");
_Static_assert(sizeof(ZipEndRecord) == 22, "ZIP end record size");
_Static_assert(sizeof(UnityArchiveMarker) == 24, "ZIP marker size");
...
#define PACK_DIR_MAGIC 0x4a524e5844495232ULL
...
static uint64_t g_pack_id;
static uint64_t g_data_checksum;
...
static int load_pair(const char *pack_path, const char *index_path,
  ...
  if (!read_at(pack_fd, &pack_header, sizeof pack_header, 0) ||
      ...
      memcmp(pack_header.magic, "JRNXPAK1", 8) ||
      memcmp(index_header.magic, "JRNXIDX1", 8) ||
...
int asset_pack_open_existing(const char *root) {
  ...
  if (!read_at(fd, &header, sizeof header, 0)) {
    free_loaded(entries, paths, fd);
    set_error("The asset pack identity could not be read");
    return 0;
  }
  g_entries = entries;
  g_paths = paths;
  g_entry_count = count;
  g_pack_fd = fd;
  g_pack_id = header.pack_id;
  g_data_checksum = header.data_checksum;
  snprintf(g_pack_path, sizeof g_pack_path, "%s", pack_path);
  g_error[0] = 0;
  return 1;
}

static int unity_archive_valid(const char *path) {
  if (!asset_pack_active() || g_entry_count > UINT16_MAX) return 0;
  int fd = open(path, O_RDONLY);
  struct stat st;
  const uint64_t tail_size = sizeof(ZipEndRecord) + sizeof(UnityArchiveMarker);
  if (fd < 0 || fstat(fd, &st) != 0 || (uint64_t)st.st_size < tail_size) {
    if (fd >= 0) close(fd);
    return 0;
  }

  ZipEndRecord end;
  UnityArchiveMarker marker;
  uint32_t first_signature = 0;
  const uint64_t end_offset = (uint64_t)st.st_size - tail_size;
  int valid = read_at(fd, &first_signature, sizeof first_signature, 0) &&
              read_at(fd, &end, sizeof end, end_offset) &&
              read_at(fd, &marker, sizeof marker,
                      end_offset + sizeof end) &&
              first_signature == 0x04034b50u &&
              end.signature == 0x06054b50u &&
              end.disk_number == 0 && end.central_disk == 0 &&
              end.disk_entries == g_entry_count &&
              end.total_entries == g_entry_count &&
              end.comment_length == sizeof marker &&
              (uint64_t)end.central_offset + end.central_size == end_offset &&
              !memcmp(marker.magic, "JRNXAPK1", 8) &&
              marker.pack_id == g_pack_id &&
              marker.data_checksum == g_data_checksum;
  close(fd);
  return valid;
}

int asset_pack_ensure_unity_archive(const char *root) {
  if (!asset_pack_active()) {
    set_error("The Unity asset archive needs an active asset pack");
    return 0;
  }
  if (g_entry_count == 0 || g_entry_count > UINT16_MAX) {
    set_error("The Unity asset archive has too many entries");
    return 0;
  }

  char archive_path[768], temp_path[768];
  snprintf(archive_path, sizeof archive_path, "%s/assets.apk", root);
  snprintf(temp_path, sizeof temp_path, "%s/assets.apk.tmp", root);
  if (unity_archive_valid(archive_path)) {
    return 1;
  }

  unlink(temp_path);
  int output = open(temp_path, O_WRONLY | O_CREAT | O_TRUNC, 0666);
  ZipEntry *zip_entries = calloc(g_entry_count, sizeof(*zip_entries));
  unsigned char *buffer = malloc(1u << 20);
  if (output < 0 || !zip_entries || !buffer) {
    if (output >= 0) close(output);
    free(zip_entries);
    free(buffer);
    unlink(temp_path);
    set_error("The Unity asset archive could not be created");
    return 0;
  }

  uint64_t total_data = 0;
  for (size_t i = 0; i < g_entry_count; i++) total_data += g_entries[i].size;
  uint64_t written_data = 0, position = 0;
  unsigned last_percent = 101;
  int ok = 1;

  for (size_t i = 0; ok && i < g_entry_count; i++) {
    const char *relative = g_paths + g_entries[i].path_offset;
    char name[1024];
    int name_length = snprintf(name, sizeof name, "assets/%s", relative);
    if (name_length <= 0 || name_length > UINT16_MAX ||
        name_length >= (int)sizeof name || g_entries[i].size > UINT32_MAX ||
        position > UINT32_MAX) {
      ok = 0;
      break;
    }

    ZipLocalHeader local = {0};
    local.signature = 0x04034b50u;
    local.version_needed = 20;
    local.method = 0;
    local.compressed_size = (uint32_t)g_entries[i].size;
    local.uncompressed_size = (uint32_t)g_entries[i].size;
    local.name_length = (uint16_t)name_length;
    zip_entries[i].local_offset = (uint32_t)position;
    zip_entries[i].size = (uint32_t)g_entries[i].size;
    if (!write_all(output, &local, sizeof local) ||
        !write_all(output, name, (size_t)name_length)) {
      ok = 0;
      break;
    }
    position += sizeof local + (uint64_t)name_length;

    uLong crc = crc32(0L, Z_NULL, 0);
    uint64_t source_offset = 0;
    while (source_offset < g_entries[i].size) {
      size_t chunk = g_entries[i].size - source_offset > (1u << 20)
                       ? (1u << 20)
                       : (size_t)(g_entries[i].size - source_offset);
      if (!read_at(g_pack_fd, buffer, chunk,
                   g_entries[i].offset + source_offset) ||
      ...
      position += chunk;
      written_data += chunk;
      unsigned percent = total_data
                           ? (unsigned)((written_data * 100) / total_data)
                           : 100;
      if (percent != last_percent) {
        char status[96];
        snprintf(status, sizeof status,
                 "Preparing Unity asset archive (first boot)\n\n  %u%%",
                 percent);
    ...
    zip_entries[i].crc = (uint32_t)crc;
    local.crc = (uint32_t)crc;
    off_t end_position = lseek(output, 0, SEEK_CUR);
    if (end_position < 0 ||
        lseek(output, zip_entries[i].local_offset, SEEK_SET) < 0 ||
        !write_all(output, &local, sizeof local) ||
        lseek(output, end_position, SEEK_SET) < 0) {
      ok = 0;
    }
  }

  uint64_t central_offset = position;
  for (size_t i = 0; ok && i < g_entry_count; i++) {
    const char *relative = g_paths + g_entries[i].path_offset;
    char name[1024];
    int name_length = snprintf(name, sizeof name, "assets/%s", relative);
    ZipCentralHeader central = {0};
    central.signature = 0x02014b50u;
    central.version_made_by = 20;
    central.version_needed = 20;
    central.method = 0;
    central.crc = zip_entries[i].crc;
    central.compressed_size = zip_entries[i].size;
    central.uncompressed_size = zip_entries[i].size;
    central.name_length = (uint16_t)name_length;
    central.local_header_offset = zip_entries[i].local_offset;
    if (!write_all(output, &central, sizeof central) ||
        !write_all(output, name, (size_t)name_length)) {
      ok = 0;
      break;
    }
    position += sizeof central + (uint64_t)name_length;
  }

  uint64_t central_size = position - central_offset;
  if (ok && (central_offset > UINT32_MAX || central_size > UINT32_MAX)) ok = 0;
  if (ok) {
    ZipEndRecord end = {0};
    UnityArchiveMarker marker = {0};
    end.signature = 0x06054b50u;
    end.disk_entries = (uint16_t)g_entry_count;
    end.total_entries = (uint16_t)g_entry_count;
    end.central_size = (uint32_t)central_size;
    end.central_offset = (uint32_t)central_offset;
    end.comment_length = sizeof marker;
    memcpy(marker.magic, "JRNXAPK1", 8);
    marker.pack_id = g_pack_id;
    marker.data_checksum = g_data_checksum;
    ok = write_all(output, &end, sizeof end) &&
         write_all(output, &marker, sizeof marker) && fsync(output) == 0;
  }
  if (close(output) != 0) ok = 0;

  if (!ok || !unity_archive_valid(temp_path)) {
    free(zip_entries);
    free(buffer);
    unlink(temp_path);
    set_error("The Unity asset archive could not be finalized");
    return 0;
  }
  unlink(archive_path);
  if (rename(temp_path, archive_path) != 0 ||
      !unity_archive_valid(archive_path)) {
    free(zip_entries);
    free(buffer);
    set_error("The validated Unity asset archive could not be installed");
    return 0;
  }
...
int asset_pack_build(const char *assets_root, const char *root) {
  ...
  memcpy(pack_header.magic, "JRNXPAK1", 8);
  ...
  memcpy(index_header.magic, "JRNXIDX1", 8);
...
int asset_pack_stat_relative(const char *path, uint64_t *size, uint64_t *ino) {
  ...
  if (ino) *ino = 0x4a52000000000000ULL | (uint64_t)(index + 1);
  return 1;
}
